...
def filter_by_stat_caps(results, current_stats):
  filtered = {}
  debug_print(f"[DEBUG] Filtering training options by stat caps. Current stats: {current_stats}")
  debug_print(f"[DEBUG] Available training options: {list(results.keys())}")
  ...
  for stat, data in results.items():
    ...
    if current_stat_value < stat_cap:
      filtered[stat] = data
      debug_print(f"[DEBUG] {stat.upper()} training allowed: current {current_stat_value} < cap {stat_cap}")
    else:
      print(f"[INFO] {stat.upper()} training filtered out: current {current_stat_value} >= cap {stat_cap}")
      debug_print(f"[DEBUG] {stat.upper()} training filtered out: current {current_stat_value} >= cap {stat_cap}")
  
  debug_print(f"[DEBUG] After stat cap filtering: {list(filtered.keys())} training options remaining")
  return filtered
