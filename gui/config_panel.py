...
class ConfigPanel(ctk.CTkFrame):
    ...
    def create_main_tab(self):
        ...
        self.device_address_var = tk.StringVar(value=cfg.get('adb_config', {}).get('device_address', '127.0.0.1:7555'))
        ...
        self.adb_path_var = tk.StringVar(value=cfg.get('adb_config', {}).get('adb_path', 'adb'))
        ...
        self.screenshot_timeout_var = tk.IntVar(value=cfg.get('adb_config', {}).get('screenshot_timeout', 5))
        ...
        self.input_delay_var = tk.DoubleVar(value=cfg.get('adb_config', {}).get('input_delay', 0.5))
        ...
        self.connection_timeout_var = tk.IntVar(value=cfg.get('adb_config', {}).get('connection_timeout', 10))
        ...
        ctk.CTkLabel(capture_frame, text="Screenshot Capture", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.colors['text_light']).pack(pady=(15, 10))

        # Method selector
        method_row = ctk.CTkFrame(capture_frame, fg_color="transparent")
        method_row.pack(fill=tk.X, padx=15, pady=5)
        ctk.CTkLabel(method_row, text="Method:", text_color=self.colors['text_light']).pack(side=tk.LEFT)
        self.capture_method_var = tk.StringVar(value=cfg.get('capture_method', 'adb'))
        ctk.CTkOptionMenu(method_row, values=['adb', 'nemu_ipc'], variable=self.capture_method_var,
                          ...
                          command=lambda _: self.toggle_nemu_settings()).pack(side=tk.RIGHT)

        # Nemu IPC settings (hidden unless selected)
        self.nemu_settings_frame = ctk.CTkFrame(capture_frame, fg_color=self.colors['bg_light'], corner_radius=10)
        nemu_cfg = cfg.get('nemu_ipc_config', {})
        # Fields
        nemu_folder_row = ctk.CTkFrame(self.nemu_settings_frame, fg_color="transparent")
        nemu_folder_row.pack(fill=tk.X, padx=15, pady=5)
        ctk.CTkLabel(nemu_folder_row, text="MuMu/Nemu Folder:", text_color=self.colors['text_light']).pack(side=tk.LEFT)
        self.nemu_folder_var = tk.StringVar(value=nemu_cfg.get('nemu_folder', 'J:\\MuMuPlayerGlobal'))
        ctk.CTkEntry(nemu_folder_row, textvariable=self.nemu_folder_var, width=320, corner_radius=8).pack(side=tk.RIGHT)

        instance_row = ctk.CTkFrame(self.nemu_settings_frame, fg_color="transparent")
        instance_row.pack(fill=tk.X, padx=15, pady=5)
        ctk.CTkLabel(instance_row, text="Instance ID:", text_color=self.colors['text_light']).pack(side=tk.LEFT)
        self.nemu_instance_var = tk.IntVar(value=nemu_cfg.get('instance_id', 2))
        ctk.CTkEntry(instance_row, textvariable=self.nemu_instance_var, width=100, corner_radius=8).pack(side=tk.RIGHT)

        display_row = ctk.CTkFrame(self.nemu_settings_frame, fg_color="transparent")
        display_row.pack(fill=tk.X, padx=15, pady=5)
        ctk.CTkLabel(display_row, text="Display ID:", text_color=self.colors['text_light']).pack(side=tk.LEFT)
        self.nemu_display_var = tk.IntVar(value=nemu_cfg.get('display_id', 0))
        ctk.CTkEntry(display_row, textvariable=self.nemu_display_var, width=100, corner_radius=8).pack(side=tk.RIGHT)

        timeout_row = ctk.CTkFrame(self.nemu_settings_frame, fg_color="transparent")
        timeout_row.pack(fill=tk.X, padx=15, pady=(5, 15))
        ctk.CTkLabel(timeout_row, text="Timeout (s):", text_color=self.colors['text_light']).pack(side=tk.LEFT)
        self.nemu_timeout_var = tk.DoubleVar(value=nemu_cfg.get('timeout', 1.0))
        ctk.CTkEntry(timeout_row, textvariable=self.nemu_timeout_var, width=100, corner_radius=8).pack(side=tk.RIGHT)

        # Initial visibility
        self.toggle_nemu_settings()
    ...
    def create_racing_tab(self):
        ...
        ctk.CTkLabel(grades_frame, text="Allowed Grades:", text_color=self.colors['text_light']).pack(anchor=tk.W)
        ...
        for i, grade in enumerate(grades_options):
            ...
            ctk.CTkCheckBox(grades_checks, text=grade, variable=var, text_color=self.colors['text_light']).pack(side=tk.LEFT, padx=(0, 12))
        ...
        ctk.CTkLabel(tracks_frame, text="Allowed Tracks:", text_color=self.colors['text_light']).pack(anchor=tk.W)
        ...
        for track in tracks_options:
            ...
            ctk.CTkCheckBox(tracks_checks, text=track, variable=var, text_color=self.colors['text_light']).pack(side=tk.LEFT, padx=(0, 10), pady=2)
        ...
        ctk.CTkLabel(distances_frame, text="Allowed Distances:", text_color=self.colors['text_light']).pack(anchor=tk.W)
        ...
        for i, dist in enumerate(distances_options):
            ...
            ctk.CTkCheckBox(distances_checks, text=dist, variable=var, text_color=self.colors['text_light']).pack(side=tk.LEFT, padx=(0, 12))
        # Strategy and Retry stay above; Custom Races section will be placed near bottom
        ...
        custom_toggle_frame = ctk.CTkFrame(racing_scroll, fg_color=self.colors['bg_light'], corner_radius=10)
        ...
        ctk.CTkCheckBox(inner_toggle, text="Do Custom Races", variable=self.do_custom_race_var, text_color=self.colors['text_light'], command=lambda: self.toggle_custom_race_settings()).pack(anchor=tk.W)
        ...
        ctk.CTkLabel(file_row, text="Custom Race File:", text_color=self.colors['text_light']).pack(side=tk.LEFT)
        ...
        ctk.CTkEntry(file_controls, textvariable=self.custom_race_file_var, width=220, corner_radius=8).pack(side=tk.LEFT, padx=(0, 5))
        ctk.CTkButton(file_controls, text="Open File", command=self.open_custom_race_file, fg_color=self.colors['accent_blue'], corner_radius=8, height=30, width=90).pack(side=tk.LEFT, padx=(0, 5))
        # Edit button below
        ctk.CTkButton(self.custom_race_settings_frame, text="Edit Custom Race List", command=self.open_custom_race_list_window, fg_color=self.colors['accent_green'], corner_radius=8, height=30, width=160).pack(anchor=tk.W, padx=0, pady=(0,5))
    ...
    def open_custom_race_list_window(self):
        ...
        try:
            ...
            # Title
            ctk.CTkLabel(window, text="Custom Race List", font=ctk.CTkFont(size=16, weight="bold"), text_color=self.colors['text_light']).pack(pady=(15, 10))
            ...
            ctk.CTkLabel(filters_frame, text="Filters", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.colors['text_light']).pack(pady=(10,5))
            ...
            def build_filter_row(parent, label_text, options, vars_map):
                ...
                ctk.CTkLabel(row, text=label_text, text_color=self.colors['text_light']).pack(side=tk.LEFT)
                checks = ctk.CTkFrame(row, fg_color="transparent")
                checks.pack(side=tk.RIGHT)
                for opt in options:
                    ctk.CTkCheckBox(checks, text=opt, variable=vars_map[opt], text_color=self.colors['text_light'], command=lambda: refresh_rows()).pack(side=tk.LEFT, padx=(0,6))
            ...
            for period in period_order:
                ...
                # Left: period label
                ctk.CTkLabel(row, text=period, text_color=self.colors['text_light'], width=260, anchor='w').pack(side=tk.LEFT, padx=10, pady=8)
    ...
    def toggle_nemu_settings(self):
        ...
        method = getattr(self, 'capture_method_var', None)
        if method and method.get() == 'nemu_ipc':
