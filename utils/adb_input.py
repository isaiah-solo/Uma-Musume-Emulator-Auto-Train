...
def run_adb_command(command):
    """Backward-compatible wrapper to utils.adb.run_adb (text mode)."""
    return run_adb(command, binary=False, add_input_delay=True)
...
def mouse_down(x, y):
    ...
    return run_adb(['shell', 'input', 'touchscreen', 'swipe', str(x), str(y), str(x), str(y), '100'])
